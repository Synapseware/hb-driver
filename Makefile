...
DEFS  = -DTWI_SPEED=800
DEFS += -DBAUD=115200
DEFS += -DUART_RX_BUFFER=128
DEFS += -DMAX_EVENT_RECORDS=12
DEFS += -DSPI_CLK_4
DEFS += -DBoard_Enterprise
DEFS +=	-DTWI_LED_DDR=DDRC
DEFS +=	-DTWI_LED_PORT=PORTC
DEFS += -DTWI_LED_PIN=PORTC0
...
sizeafter:
	@if test -f $(TARGET).elf; then echo $(MSG_SIZE_AFTER); $(ELFSIZE); \
	2>/dev/null; fi
